...
def get_pair_statistics(vocab):
    ...
    return stats, indices,astats

import numpy
def get_max_variance(astats):
    char_variance = {}
    for char in astats:
        list1 = []
...
def learn_bpe(infile, outfile, num_symbols, min_frequency=2, verbose=False, is_dict=False, total_symbols=False, num_workers=1):
    ...
    for i in range(num_symbols):
        stats, indices,astats = get_pair_statistics(sorted_vocab)
        #print(len(stats), len(astats))
        #print(sorted_vocab)
        if stats:
            pair = get_max_variance(astats)
        #print(pair)
        ...
        changes = replace_pair(tuple(pair), sorted_vocab, indices)
