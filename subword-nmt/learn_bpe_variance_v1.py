...
def get_pair_statistics(vocab, update=False, first=None, second=None,astats={}, indices=defaultdict(lambda:defaultdict(int)), added_bpe={}):
    ...
    for i, (word, freq) in enumerate(vocab):
        ...
        if update == True: bigram = "".join([first, second])
    
        if update == True:
           if first not in word  and  second not in  word and bigram not in word: 
                #print("first: ", first, "second: ", second, "word", word)
                continue
        ...
        for j, char in enumerate(word):
            ...
            if j == 0 and j+1 < len(word):
                ...
                if update == True and  (first !=  char and second !=  char and first != after_token and second != after_token and bigram != char and bigram != after_token): continue
                if " ".join([char, after_token]) not in astats[char]:
                   #if " ".join([char, after_token]) in added_bpe:
                   #    print(word, char, after_token, "wrong word")
                ...
                if update == True and (first !=  char and second != char and first != prev_char and second != prev_char and bigram != char and bigram != prev_char): continue
                if " ".join([prev_char, char]) not in astats[char]:
                   #if " ".join([prev_char, char]) in added_bpe:
                   #    print(word, char, prev_char, "wrong word")
                ...
                if char not in astats:
                    astats[char]={}
                if update == False or first == char or second == char or first == prev_char or second == prev_char or bigram == char or bigram == prev_char:
                    if " ".join([prev_char, char]) not in astats[char]:
                        astats[char][" ".join([prev_char, char])] = 0
                    #if " ".join([prev_char, char]) in added_bpe:
                    #    print(word, char, prev_char, "wrong word")
                    astats[char][" ".join([prev_char, char])] += 1
                    indices[prev_char, char][i] += 1
                if update == False or first == char or second == char or first == after_token or second == after_token or bigram == char or bigram == after_token:
                    if " ".join([char, after_token]) not in astats[char]:
                        astats[char][" ".join([char, after_token])] = 0
                    astats[char][" ".join([char, after_token])] += 1
                    indices[char, after_token][i] += 1
...
def get_max_variance(astats, flag="variance"):
    ...
    for char in astats:
        ...
        if flag == "variance":
            if len(list1) >= 2: char_variance[char] = numpy.std(list1)
        if flag == 'frequency':
            if len(list1) >= 2: char_variance[char] = sum(list1) 
        if "entropy" in flag:
            entropy = 0
    ...
    #print(max_index)
    #if max_index == None:
    #   print(len(astats), len(char_variance))
    #   print(flag=="variance")
...
def replace_pair(pair, vocab, indices,  astats):
    ...
    #print("delete test before", len(astats), first in astats, second in astats)
    if first in astats:  del astats[first]
    if second in astats: del astats[second]
    #print("delete_Test", len(astats))
    for item in astats:
...
def learn_bpe(infile, outfile, num_symbols, min_frequency=2, verbose=False, is_dict=False, total_symbols=False, num_workers=1, flag="variance"):
    ...
    for i,word in enumerate(sorted_vocab):
        if word[1] < 2:
            length = i
            break
    sorted_vocab = sorted_vocab[:length]
    #sorted_vocab = vocab
    ...
    for i in range(num_symbols):
        if i == 0:
          ...
          indices, astats = get_pair_statistics(sorted_vocab, True, pair[0], pair[1], astats, indices, added_bpe)
        #print(len(stats), len(astats))
        #print(sorted_vocab)
        
        pair = get_max_variance(astats,flag)
        added_bpe[" ".join([pair[0],pair[1]])]=0
        #print(pair)
        ...
        #print(len(astats), "replace before")
        changes = replace_pair(tuple(pair), sorted_vocab, indices, astats)
        #print(len(astats), "replace after")
