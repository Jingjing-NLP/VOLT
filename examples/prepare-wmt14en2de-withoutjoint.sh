...
for l in $src $tgt; do
    rm -rf $TRAIN$l
    cat $tmp/train.$l > $TRAIN$l
done
#fi

echo "learn_bpe.py on ${TRAIN}..."
python3 $BPEROOT/learn_bpe.py -s $BPE_TOKENS  < $TRAIN$src > $BPE_CODE$src
python3 $BPEROOT/learn_bpe.py -s $BPE_TOKENS  < $TRAIN$tgt > $BPE_CODE$tgt


for L in $src $tgt; do
    for f in train.$L valid.$L test.$L; do
        echo "apply_bpe.py to ${f}..."
        python3 $BPEROOT/apply_bpe.py -c $BPE_CODE$L < $tmp/$f > $tmp/bpe.$f
    done
done
