...
def sinkhorn(a, b, M, reg, method='sinkhorn', numItermax=1000,
    ...
    if method.lower() == 'sinkhorn':
        return sinkhorn_knopp(a, b, M, reg, numItermax=numItermax,
                              ...
                              **kwargs)
    elif method.lower() == 'mysinkhorn':
        return mysinkhorn_knopp(a, b,  M, l, reg, numItermax=numItermax,
                              stopThr=stopThr, verbose=verbose, log=log,
                              **kwargs)
...
def sinkhorn_knopp(a, b, M, reg, numItermax=1000,
    ...
    if n_hists:  # return only loss
        ...
        if log:
            ...
            p_matrix = u.reshape((-1, 1)) * K * v.reshape((1, -1))
            sum1 = 0
            for item in p_matrix:
                for word in item:
                    #print(word)
                    sum1 += word * math.log(1+float(word))
            return sum1, u.reshape((-1, 1)) * K * v.reshape((1, -1)) #sum1



def mysinkhorn_knopp(a, b, M, l, reg, numItermax=1000,
                   stopThr=1e-9, verbose=False, log=False, **kwargs):
    r"""
    Solve the entropic regularization optimal transport problem and return the OT matrix
    The function solves the following optimization problem:
    ...
    #N = np.ones((M.shape[0], M.shape[1]), dtype=np.float64)
    ...
    #print(l.shape())
    lpow = np.asarray([np.power(item,item) for item in l])
    # Next 3 lines equivalent to K= np.exp(-M/reg), but faster to compute
    K = np.empty(M.shape, dtype=M.dtype)
    np.divide(M, -reg, out=K)
    np.exp(K, out=K)
    ...
    # print(np.min(K))
    tmp2 = np.empty(b.shape, dtype=M.dtype)
    #print(np.tile(np.expand_dims(a,axis=1),b.shape[0]).shape)
    #print(a.shape, (np.power(a,l)/lpow).shape)
    Kp = (1 / (np.power(np.tile(np.expand_dims(a,axis=1),b.shape[0]),np.tile(np.expand_dims(np.log(l),axis=1),a.shape[0]).T) / np.tile(np.expand_dims(l,axis=1),a.shape[0]).T)) * K
    cpt = 0
    err = 1
    #print((np.tile(np.expand_dims(lpow,axis=1),a.shape[0]).T).shape,(np.power(np.tile(np.expand_dims(u,axis=1),b.shape[0]),np.tile(np.expand_dims(l,axis=1),a.shape[0]).T)).shape)
    while (cpt < numItermax):
        uprev = u
        vprev = v
       
        KtransposeU = np.dot(K.T, u) #np.dot(K.T, np.power(np.tile(np.expand_dims(u,axis=1),b.shape[0]),np.tile(np.expand_dims(l,axis=1),a.shape[0]).T)/np.tile(np.expand_dims(lpow,axis=1),a.shape[0]).T)
        print(cpt, err) 
        new_pij = np.einsum('i,ij,j->ij', u, K, v)
        #for i in range(len(new_pij)):
        #    for j in range(len(new_pij[i])):
        #        new_pij[i][j] = np.power(new_pij[i][j], 1/l[j])
        #new_b = [b[j] if abs(sum(new_pij[:,j])-b[j]) < abs(sum(new_pij[:,j])) else 0 for j in range(len(b))]
        #new_b = [0 if math.pow(min(new_pij[:,j])-b[j],2) < math.pow(min(new_pij[:,j]),2) else b[j] for j in range(len(b))]
        ...
        #           avg += N[i][j]
        #    avg /= num
        #    N[:,j] = [avg if new_pij[i][j] != 0 else 0 for i in range(len(a))] 
        v = np.divide(np.power(b,np.log(l))/l, KtransposeU)
    ...
    if n_hists:  # return only loss
        ...
        if log:
            ...
            #p_matrix = np.power(u.reshape((-1, 1)) * K * v.reshape((1, -1))#,1/np.tile(np.expand_dims(np.log(l),axis=1),a.shape[0]).T)
            sum1 = 0
            for item in p_matrix:
                for word in item:
                    sum1 += word * math.log(word)
            return sum1#np.power(u.reshape((-1, 1)) * K * v.reshape((1, -1)),1/np.tile(np.expand_dims(np.log(l),axis=1),a.shape[0]).T)
